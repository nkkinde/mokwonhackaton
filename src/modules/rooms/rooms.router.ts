...
r.post('/', authGuard, async (req: AuthedRequest, res) => {
  ...
  const room = await prisma.room.create({ data: { name, type, createdBy: req.user!.id } });
  await prisma.roomMember.create({ data: { roomId: room.id, userId: req.user!.id, role: 'owner' } });
  res.json(room);
});
